...
export function useAccountOpening(ticketId: MaybeRefOrGetter<string | number>) {
  ...
  const videoVerificationRequired = computed(() => {
    ...
    // banner could demand video while the Actions menu offered a call.
    if (record.value?.case?.videoVerificationRequired) return true;

    // Otherwise derive it the way the server does on save: from the date of
    // birth the case will hold. The form value first, so the banner previews the
    // edit in progress; then what is stored; then the application's, which is
    // what a case with no date of birth of its own inherits.
    //
    // Deliberately NOT `application.videoVerificationRequired`: that is the
    // vendor's own flag, invisible to the workflow, and using it was what let the
    // two disagree.
    const dateOfBirth =
      form.clientDateOfBirth ||
      record.value?.verification?.clientDateOfBirth ||
      record.value?.application?.dateOfBirth ||
      "";

    return isVideoVerificationRequired(computeAge(dateOfBirth));
  });
