/**
 * Resolves a ticket to its closure case name.
 *
 * The ticket header needs this to drive the Actions menu, and it has to work
 * whichever tab is open — so it cannot rely on the Closure tab being mounted.
 * Deliberately minimal: the name and nothing else. The tab itself loads the full
 * case through `useAccountClosure`.
 */
import { computed, ref, toValue, watch, type MaybeRefOrGetter } from "vue";

import { fetchCase } from "./api";

export function useClosureCase(
...
export type UseClosureCase = ReturnType<typeof useClosureCase>;
