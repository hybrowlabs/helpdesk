/**
 * Resolves a ticket to its modification case name.
 *
 * The ticket header needs this to drive the Actions menu, and it has to work
 * whichever tab is open — so it cannot rely on the Modification tab being
 * mounted. Deliberately minimal: the name and nothing else. The tab itself
 * loads the full case through `useClientModification`.
 */
import { computed, ref, toValue, watch, type MaybeRefOrGetter } from "vue";

import { fetchCase } from "./api";

export function useModificationCase(
...
export type UseModificationCase = ReturnType<typeof useModificationCase>;
